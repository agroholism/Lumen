	...
	public class IdExpression : Expression {
		...
		public Value Eval(Scope e) {
			if (!e.IsExsists(this.id)) {
				throw new StandartLibrary.Exception($"unknown identifiter '{this.id}'", stack: e) { file = this.file, line = this.line };
			}
		...
		public Expression Closure(List<String> visible, Scope thread) {
			if (!(visible.Contains(this.id))) {
				if (!thread.IsExsists(this.id)) {
					throw new StandartLibrary.Exception($"unknown identifiter '{this.id}'", stack: thread) { file = this.file, line = this.line };
				}
