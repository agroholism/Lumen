		...
		public Value Eval(Scope e) {
			...
			if (obj is Expando) {
				...
				if (!module.Contains(((DotExpression)res).nameVariable))
					throw new StandartLibrary.Exception("модуль не содержит функцию '" + ((DotExpression)this.res).nameVariable + "'", stack: e);
				...
				if (!type.Contains(((DotExpression)this.res).nameVariable)) {
					if (type.AttributeExists(((DotExpression)this.res).nameVariable)) {
						...
						fun = new LambdaFun((ex, args) => {
							Value ths = args[0];
							if (ths.Type != type) {
								throw new StandartLibrary.Exception("wrong type",stack: ex);
							}
							ex.Set("this", ths);
				...
				StandartLibrary.KType cls = obj.Type;
				...
				if (!cls.AttributeExists(name)) {
					try {
						...
						throw new StandartLibrary.Exception("тип не содержит функцию '" + name + "'", stack: e);
					}
				}
