	...
	public class CombineAssigment : Expression {
		...
		public Value Eval(Scope scope) {
			if (scope.IsConstant(this.id)) {
				throw new StandartLibrary.Exception($"cannot change constant '{this.id}'", stack: scope) {
					line = this.line,
					file = this.file
				};
			}
			...
			if (!scope.ExistsInThisScope(this.id)) {
				throw new StandartLibrary.Exception($"variable '{this.id}' is not declared", stack: scope) {
					line = this.line,
					file = this.file
				};
			}
