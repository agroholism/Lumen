	...
	internal sealed class Lexer {
					...
					switch (current) {
						...
						case 'u':
							if (Peek(1) != '{') {
								...
								while (true) {
									if (current == '"') {
										throw new StandartLibrary.Exception("consumed } with \\u ", stack: null) {
											line = this.line,
											file = this.file
										};
									}
		...
		private void CheckOutOfRange() {
			if (this.position >= this.length) {
				throw new StandartLibrary.Exception("consumed symbol '\"'") {
					line = this.line,
					file = this.file
				};
			}
			...
			while (true) {
				if (current == '.') {
					...
					if (buffer.ToString().IndexOf('.') != -1)
						throw new StandartLibrary.Exception("лишняя точка < литерал num", stack: null);
				}
				else if (current == '_') {
					current = Next();
					continue;
