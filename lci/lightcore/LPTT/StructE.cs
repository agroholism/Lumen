		...
		//	var def = null;// new Initialize(arg.args);
			...
			if (isException) {
				result.Set("new", new LambdaFun((scope, argsx) => {
					StandartLibrary.Exception obj = new StandartLibrary.Exception(argsx[0].ToString(scope), result);
			...
			foreach (Value i in result.includedModules) {
				if (i is Module hi) {
					if (!hi.TypeImplemented(result)) {
						throw new StandartLibrary.Exception($"класс {this.name} не удовлетворяет требованиям модуля {hi.name}", stack: e);
					}
				}
			}
