...
namespace Stereotype {
	internal sealed partial class Parser {
			...
			if (Match(TokenType.DO)) {
				while (!Match(TokenType.END)) {
					if (Match(TokenType.EOF)) {
						throw new StandartLibrary.Exception("end of file (type parsing)") {
							line = this.line,
							file = this.fileName
						};
					}
		...
		private Expression AloneORBlock() {
			...
			while (!Match(TokenType.END)) {
				if (Match(TokenType.EOF)) {
					throw new StandartLibrary.Exception("пропущена закрывающая фигурная скобка", stack: Interpriter.mainScope) {
						line = line,
						file = this.fileName
					};
				}
		...
		private Expression Is() {
			...
			while (Match(TokenType.FPIPE)) {
				expr = new Applicate(Range(), new List<StandartLibrary.Expressions.Expression> { expr }, this.line);
			}
		...
		private Expression Application() {
			...
			while (Match(TokenType.LPAREN)) {
				...
				while (!Match(TokenType.RPAREN)) {
					...
					Match(TokenType.SPLIT);
					if (Match(TokenType.EOF)) {
						throw new StandartLibrary.Exception("при вызове функции пропущен токен ')'", stack: Interpriter.mainScope);
					}
				}
		...
		private Expression Primary() {
			...
			if (Match(TokenType.NEW)) {
				...
				List<Expression> args = new List<StandartLibrary.Expressions.Expression>();
				...
				List<Expression> initor = new List<StandartLibrary.Expressions.Expression>();
				if (Match(TokenType.DO)) {
					while (!Match(TokenType.END)) {
		...
		private Expression ParseType() {
			...
			while (Match(TokenType.LBRACKET)) {
				List<Expression> exps = new List<StandartLibrary.Expressions.Expression>();
				while (!Match(TokenType.RBRACKET)) {
					exps.Add(Expression());
					Match(TokenType.SPLIT);
				}
		...
		private List<ArgumentMetadataGenerator> ParseArgs(TokenType border) {
			...
			while (!Match(border)) {
				...
				if (Match(TokenType.EOF)) {
					throw new StandartLibrary.Exception("пропущена закрывающая круглая скобка", stack: Interpriter.mainScope);
				}
			}
		...
		private Token Consume(TokenType type) {
			...
			if (type != Current.Type) {
				throw new StandartLibrary.Exception($"ожидался токен " + type.ToString() + " встречен " + Current.Type.ToString(), stack: Interpriter.mainScope) {
					line = this.line,
					file = this.fileName
				};
			}
