...
namespace Lumen.Light {
    internal sealed class Parser {
        ...
        private IPattern ParsePattern() {
            ...
            if (this.LookMatch(0, TokenType.NUMBER)) {
                return new ValuePattern(new Number(Double.Parse(this.Consume(TokenType.NUMBER).Text)));
            }
            ...
            if (this.LookMatch(0, TokenType.WORD)) {
                ...
                while (!this.LookMatch(0, TokenType.EQUALS)) {
                    ...
                    if (this.LookMatch(0, TokenType.NUMBER)) {
                        subPatterns.Add(new ValuePattern(new Number(Double.Parse(this.Consume(TokenType.NUMBER).Text))));
                    }
                }
        ...
        private IPattern ParseArgPattern() {
            ...
            if (this.Match(TokenType.VOID)) {
                ...
                if (id == "_" && this.LookMatch(0, TokenType.EQUALS)) {
                    ...
                    if (LookMatch(1, TokenType.CONTEXT)) {
                        String contextParam = this.Consume(TokenType.WORD).Text;
                        Match(TokenType.CONTEXT);
                        result = new ContextPattern(id, contextParam, ParseArgPattern());
                    } else {
                        while (!this.LookMatch(0, TokenType.RPAREN) && !LookMatch(0, TokenType.CONTEXT)) {
        ...
        private Expression ParseVariableDeclaration(String id, Boolean isMutable) {
            ...
            return new VariableDeclaration(id, expression, isMutable, this.fileName, this.line);
        }
        ...
        private Expression ParseDeclaration() {
            ...
            if (this.LookMatch(0, TokenType.EQUALS)) {
                return this.ParseVariableDeclaration(name, isMutable);
            }
        ...
        private Expression Is() {
            ...
            while (this.Match(TokenType.FUNCTORBIND)) {
                expr = new DotApplicate(
                    new DotExpression(expr, ">>-", this.fileName, this.line), new List<Expression> { this.Range() });
            }
        ...
        private Expression Slice(Expression inn) {
            ...
            while (this.LookMatch(0, TokenType.LBRACKET)) {
                ...
                if (this.Match(TokenType.ASSIGN)) {
                    ...
                    return new DotApplicate(new DotExpression((res as GetIndexE).res, Op.SETI, this.fileName, this.line), args);
                }
            }
