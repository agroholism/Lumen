...
namespace Lumen.Lang.Std {
	internal sealed class RBigNum : Record {
		internal RBigNum() {
			...
			*/
			...
			SetAttribute("to_s", new LambdaFun((scope, args) => {
				...
				if (rat) {
					return new Str(num.ToBigFloat(scope).ToRationalString());
				}

				if (ct != null) {
					return new Str(num.ToBigFloat(scope).ToString((Int32)ct.ToDouble(scope)));
				}
				...
				Arguments = new List<FunctionArgument> {
						...
						new FunctionArgument("ct", Const.VOID),
